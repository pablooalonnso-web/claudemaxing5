 ...
 *   - a bounded loss on every exit back to USDG;
 ...
 * everything the allocator holds (idle USDG plus shares of every target
 * vault), so exits never depend on a price, a swap or the keeper. Anyone can
 * then exit the vault shares through the normal vault flow.
 *
 * Every parameter change goes through a 24 hour review window, except the
 * changes that only reduce risk (pause, disable a target, lower the cap),
 * which apply at once.
 *
...
contract VertexAllocatorV1 {
    ...
    uint16 public constant MAX_EXIT_LOSS_BPS = 500; // 5%
    ...
    uint16 public maxExitLossBps; // an exit to USDG must return at least (1 - this) of the position value
    ...
    event MaxExitLossSet(uint16 bps);
    ...
    constructor(address usdg, address owner_, address keeper_, address guardian_, address treasury_, uint256 depositCap_, uint16 depositFeeBps_, address[] memory initialTargets, uint16[] memory initialWeights) {
        ...
        if (initialTargets.length != initialWeights.length) revert TooManyTargets();
        USDG = IERC20(usdg);
        owner = owner_;
    ...
    /// Redeem shares in kind: the pro rata slice of idle USDG and of every target position. Never needs a price.
    function withdraw(uint256 shares, address receiver) external nonReentrant returns (uint256 idleOut) {
        if (shares == 0) revert ZeroAmount();
        if (receiver == address(0)) revert ZeroAddress();
        ...
        for (uint256 i = 0; i < n; i++) {
            ...
            uint256 held = IManagedVault(vault).balanceOf(address(this));
            uint256 part = (held * shares) / supply;
            if (part > 0) {
    ...
    function allocate(address vault, IRouter.Entry calldata entry) external nonReentrant whenNotPaused onlyKeeper returns (uint256 vaultShares) {
        ...
        uint256 total = totalAssets();
        if ((valueOf(vault) + entry.budget) * 10_000 > total * t.maxWeightBps) revert WeightExceeded();
        ...
        vaultShares = v.balanceOf(address(this)) - before;
        emit Allocated(vault, entry.budget, vaultShares);
    }
    ...
    function deallocate(address vault, uint256 vaultShares, uint256 minimum, uint256 deadline, uint256 configuration, IRouter.ExitSwap calldata swap) external nonReentrant onlyKeeper returns (uint256 assetsOut) {
        ...
        if (minimum * 10_000 < expected * (10_000 - maxExitLossBps)) revert ExitFloorTooLow();
    ...
    function _setTarget(address vault, uint16 maxWeightBps) internal {
        ...
        targets[vault] = Target({enabled: true, maxWeightBps: maxWeightBps, router: router, valuation: valuation});
    ...
    function setMaxExitLoss(uint16 bps) external onlySelf {
        if (bps > MAX_EXIT_LOSS_BPS) revert LossTooHigh();
        maxExitLossBps = bps;
        emit MaxExitLossSet(bps);
    }
