...
/** STEP 03 — the fee split: 70 compounds, 10 operations, 20 buyback. */
  ...
  return (
    <svg viewBox="0 0 600 400" className="home-art" aria-hidden="true">
      ...
      <g fontFamily="var(--font-mono), monospace" fontSize="11" letterSpacing="1">
        ...
        <rect x="400" y="70" width="120" height="44" fill="#FFCFFE" />
        <text x="460" y="90" textAnchor="middle" fill="#3D3B4F" fontWeight="700">
          10%
        </text>
        <text x="460" y="105" textAnchor="middle" fill="#3D3B4F">
          OPERATIONS
        </text>
        <rect x="240" y="300" width="120" height="44" fill="#DAFF01" />
        <text x="300" y="320" textAnchor="middle" fill="#3D3B4F" fontWeight="700">
          20%
        </text>
        <text x="300" y="335" textAnchor="middle" fill="#3D3B4F">
          BUYBACK · BURN
        </text>
      ...
      <g fill="none" stroke="#C5FFD6" strokeWidth="1.2" strokeDasharray="5 5" className="g-marching">
        ...
        <path d="M300 180 L 400 114" />
        <path d="M300 220 L 300 300" />
      </g>
      <g fill="#28E99F">
...
export function StackArt({ src, label, tint = "#C5FFD6" }: { src: string; label: string; tint?: string }) {
  return (
    <div className="home-stack-art" style={{ background: "#EEEEEE" }}>
      <svg viewBox="0 0 600 400" className="home-art" aria-hidden="true">
        <rect width="600" height="400" fill="#EEEEEE" />
        <Grid color="#5882FF" size={24} opacity={0.14} />
        <g fill="none" stroke="#5882FF" strokeWidth="0.8" opacity="0.6">
          <path d="M60 200 C 200 60, 400 340, 540 200" />
          <path d="M60 200 C 200 340, 400 60, 540 200" />
          <circle cx="300" cy="200" r="120" strokeDasharray="4 6" />
        </g>
        <rect x="200" y="120" width="200" height="160" fill={tint} opacity="0.8" />
        <g fill="#FFACFE">
          <rect x="196" y="116" width="8" height="8" />
          <rect x="396" y="116" width="8" height="8" />
          <rect x="196" y="276" width="8" height="8" />
          <rect x="396" y="276" width="8" height="8" />
        </g>
      </svg>
      <span className="home-stack-mark">
        <Image src={src} alt="" width={64} height={64} loading="eager" unoptimized />
        <small>{label}</small>
      </span>
    </div>
  );
...
/** The blue wave field behind the strategies band. */
export function WaveArt() {
  const lines = [];
  for (let i = 0; i < 26; i++) {
    const y = 90 + i * 11;
    const amp = 28 - Math.abs(i - 13) * 1.4;
    lines.push(
      <path
        key={i}
        d={`M ${140 - i * 4} ${y} C 300 ${y - amp}, 420 ${y + amp}, 560 ${y - amp * 0.6} S 820 ${y + amp}, 980 ${y - amp * 0.4} S 1240 ${y + amp * 0.5}, ${1300 + i * 4} ${y}`}
        fill="none"
        stroke="#5882FF"
        strokeWidth="1.2"
        opacity={0.55 - Math.abs(i - 13) * 0.012}
      />,
    );
  }
  return (
    <svg viewBox="0 0 1440 460" className="home-wave" preserveAspectRatio="xMidYMid slice" aria-hidden="true">
      {lines}
      <g fontFamily="var(--font-mono), monospace" fontSize="10" letterSpacing="1.5" fill="#5882FF" opacity="0.7">
        {["LONG", "SHORT", "HEDGE", "REBALANCE", "LONG", "SHORT", "HEDGE"].map((t, i) => (
          <text key={i} x="170" y={130 + i * 14}>
            {t}
          </text>
        ))}
      </g>
      <g fill="#DAFF01">
        <rect x="500" y="204" width="8" height="8" />
        <rect x="760" y="176" width="8" height="8" />
        <rect x="920" y="238" width="8" height="8" />
        <rect x="1080" y="150" width="8" height="8" />
      </g>
      <g fill="none" stroke="#FFACFE" strokeWidth="1" strokeDasharray="4 5">
        <path d="M504 208 L 640 320 L 764 180" />
        <path d="M924 242 L 1000 300 L 1084 154" />
      </g>
      <g transform="translate(1180 120)" fill="none" stroke="#DAFF01" strokeWidth="1">
        <rect x="-60" y="-50" width="120" height="100" transform="rotate(12)" strokeDasharray="3 4" />
        <circle r="34" strokeDasharray="2 3" />
        <circle r="24" />
      </g>
      <g transform="translate(1120 250)" fill="#DAFF01" opacity="0.8">
        {Array.from({ length: 12 }).map((_, i) => (
          <rect key={i} x={i * 12} y={0} width="6" height="34" opacity={0.3 + (i % 4) * 0.2} />
        ))}
      </g>
    </svg>
  );
...
/** The CTA diagram: a vault card fanned out to fee nodes. */
    ...
    <svg viewBox="0 0 600 440" className="home-cta-art" aria-hidden="true">
      ...
      <g transform="translate(150 130)">
        ...
        <text x="12" y="150" fontFamily="var(--font-sans), sans-serif" fontSize="10" fill="#3D3B4F">
          Fees claimed. 70% compounded, 20% reserved for buyback.
        </text>
      </g>
    </svg>
  );
